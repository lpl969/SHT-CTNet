...
def build_dataset_from_h5(
    ...
    for i in range(data_norm.shape[0]):
        ...
        Xc = add_awgn_mics_freq(Xc, snr_db=snr_db)

        if use_sht:

            Xc_r = np.real(Xc); Xc_i = np.imag(Xc)
            x_in = np.concatenate([Xc_r, Xc_i], axis=0).astype(np.float32)


            x_t = torch.from_numpy(x_in[None, ...]).to(device)
            y_t = proj(x_t)
            y = y_t[0].detach().cpu().numpy().astype(np.float32)
            ...
            if sht_representation == 'power':
                ...
                Yr, Yi = y[:M2], y[M2:]
                P = Yr**2 + Yi**2
                S = np.log10(P + 1e-12).astype(np.float32)

                S_crop = np.stack([stft._spec_crop(S[k]) for k in range(S.shape[0])], axis=0)
                X_list.append(S_crop)
            else:
