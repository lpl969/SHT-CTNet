...
class Attention(nn.Module):
    def __init__(self, dim, window_size=None, num_heads=4, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.dim = dim
...
class SE_Module(nn.Module):
    def __init__(self, channel, ratio=16):
        ...
        self.fc1 = nn.Conv2d(channel, reduced_channels, kernel_size=1)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.Conv2d(reduced_channels, channel, kernel_size=1)
...
class LFAEncoder(nn.Module):
    def __init__(self, dim, drop_path=0., expan_ratio=2.0, kernel_s=7, downsample=True):
        super().__init__()
        ...
        hidden_dim = int(expan_ratio * dim)
...
class LFAStokenNetwork(nn.Module):
    def __init__(self, dim, stoken_size, n_iter=1, num_heads=4, qkv_bias=False, qk_scale=None, attn_drop=0.,
                 proj_drop=0., drop_path=0., expan_ratio=2.0, kernel_s=7, downsample=True):
        super().__init__()
        self.encoder = LFAEncoder(dim, drop_path, expan_ratio, kernel_s, downsample=downsample)
...
class mynet(nn.Module):
    def __init__(self, num_classes, in_ch=1, base_ch=8):
        super().__init__()
        mid_ch = base_ch * 2
        self.conv = nn.Conv2d(in_ch, base_ch, kernel_size=7, stride=2, padding=3)
        self.conv2 = nn.Conv2d(base_ch, base_ch, kernel_size=3, stride=2, padding=1)
        self.BN = nn.BatchNorm2d(base_ch)
        self.relu = nn.ReLU(inplace=True)
        self.LS = LFAStokenNetwork(base_ch, stoken_size=[4, 4], num_heads=4, expan_ratio=2.0, downsample=False)
        self.conv1 = nn.Conv2d(base_ch, mid_ch, kernel_size=3, stride=2, padding=1)
        self.BN1 = nn.BatchNorm2d(mid_ch)
        self.LS1 = LFAStokenNetwork(mid_ch, stoken_size=[8, 8], num_heads=4, expan_ratio=2.0, downsample=False)
