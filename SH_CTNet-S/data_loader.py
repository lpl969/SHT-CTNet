...
from sht import build_sht_from_xyz, SHTProjection, n_sh_from_order
...
class STFTSpectrogram():
    ...
    def batch_spec_crop(self, tensor: np.ndarray) -> np.ndarray:
        cropped_samples = []
        for sample in tensor:
            cropped_channels = [self._spec_crop(sample[idx]) for idx in range(sample.shape[0])]
            cropped_samples.append(np.stack(cropped_channels, axis=0))
        return np.stack(cropped_samples, axis=0)
...
def build_dataset_from_h5(
    ...
    assert sht_representation in ('power', 'complex')
    ...
    if use_sht:
        ...
        # A, A_dag = build_sht_from_xyz(xyz, L=L, lam=1e-3, device=device)
        # proj = SHTProjection(A_dag).to(device)
        proj_dtype = torch.float16 if torch.cuda.is_available() else torch.float32
        A, A_dag = build_sht_from_xyz(xyz, L=L, lam=1e-3, device=device, dtype=proj_dtype)
        proj = SHTProjection(A_dag).to(device, dtype=proj_dtype)
        def _project_specs(xc_batch: np.ndarray) -> np.ndarray:
            xr = np.real(xc_batch).astype(np.float32)
            xi = np.imag(xc_batch).astype(np.float32)
            x_in = np.concatenate([xr, xi], axis=1)
            x_t = torch.from_numpy(x_in).to(device=device, dtype=proj_dtype)
            with torch.no_grad():
                y = proj(x_t).to(torch.float32)
            return y.cpu().numpy()
        M = n_sh_from_order(L)
        in_ch = (2*M) if (sht_representation == 'complex') else M
    else:
        in_ch = 1
    ...
    for start in range(0, num_samples, batch_size):
        ...
        for idx in range(start, end):
            ...
            spec = stft.gen_stft_complex(data_norm[idx], win_len=n_fft, overlap=hop)
            spec = simulate_array_freq_phase_only(
                    spec, xyz, doa_theta=doa_theta_i, doa_phi=doa_phi_i,
                    carrier_hz=carrier_hz, wave_speed=wave_speed)
            snr_db = pick_snr(split, snr_train_range_db, snr_eval_range_db)
            spec = add_awgn_mics_freq(spec, snr_db=snr_db).astype(np.complex64)
            specs_batch.append(spec)

        if use_sht and specs_batch:
            specs_np = np.stack(specs_batch, axis=0)
            projected = _project_specs(specs_np)
            if sht_representation == 'power':
                M2 = projected.shape[1] // 2
                Yr, Yi = projected[:, :M2], projected[:, M2:]
                power = np.log10(np.maximum(Yr**2 + Yi**2, 1e-12)).astype(np.float32)
                crops = stft.batch_spec_crop(power)
            else:
                crops = stft.batch_spec_crop(projected.astype(np.float32))
            for sample_crop in crops:
                X_list.append(sample_crop)
        elif specs_batch:
            for spec in specs_batch:
                X_used = spec.mean(axis=0)
                S = stft.to_log_power(X_used, eps=0.0)
                S = stft._spec_crop(S)
